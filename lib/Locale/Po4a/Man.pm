...
sub this_macro_needs_args {
    ...
    unless (length($args)) {
	die sprintf(gettext(
"%s: macro %s called with no args. I know that in this case,\n".
" next line should be taken as argument (see man(7)), but it\n".
" would make the po4a parser too complicate.\n".
...
#		$self->pushline(".br\n") unless (length($paragraph));
		...
		if ($line =~ /Pod::Man/) {
		    ...
		    warn gettext("This file was generated with help2man. Translate the source file with the regular gettext for source files.\n");
...
$macro{'de'}=sub {
    die gettext("This page define a new macro with '.de'. Since I'm not a real groff parser yet, I can't handle this.\n");
};
...
$macro{'ie'}=$macro{'if'}=sub {
    die sprintf(gettext("This page uses conditionals with '%s'. Since I'm not a real groff parser yet, I can't handle this.\n"),$_[1]);
};

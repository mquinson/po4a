...
use constant {
    ...
    NESTED_BLOCK_NODE_TYPES         => [ CommonMark::NODE_BLOCK_QUOTE, CommonMark::NODE_LIST, CommonMark::NODE_ITEM ],
    INLINE_CONTENT_BLOCK_NODE_TYPES => [ CommonMark::NODE_PARAGRAPH,   CommonMark::NODE_HEADING ],
    LITERAL_BLOCK_NODE_TYPES        => [ CommonMark::NODE_CODE_BLOCK,  CommonMark::NODE_HTML_BLOCK ],
};
...
sub initialize {
    ...
    $self->{_commonmark_options}{yfm_keys} =
      $self->_parse_comma_separated_option( $options{yfm_keys} );
    $self->{_commonmark_options}{yfm_paths} =
      $self->_parse_comma_separated_option( $options{yfm_paths} );
    return $self->SUPER::initialize;
}

sub _parse_comma_separated_option {
    my ( $self, $option ) = @_;
    my $result = {};
    if ($option) {
        for ( split( ',', $option ) ) {
...
sub parse {
    ...
    $self->debug
      and warn wrap_mod(
        'po4a::commonmark',
        dgettext( 'po4a', "Version information of libcmark: %s (compile time: %s)", ),
    ...
    while ( my ( $event_type, $node ) = $iterator->next ) {
        ...
        if ( grep { $node->get_type == $_ } @{ (NESTED_BLOCK_NODE_TYPES) } ) {
            $self->_update_container_stack( { event_type => $event_type, node => $node } );
        } elsif ( grep { $node->get_type == $_ } @{ (INLINE_CONTENT_BLOCK_NODE_TYPES) } ) {
            my $line = $self->_update_container_stack( { event_type => $event_type, node => $node } );
        ...
        } elsif ( grep { $node->get_type == $_ } @{ (LITERAL_BLOCK_NODE_TYPES) } ) {
            ( $node->get_type == CommonMark::NODE_CODE_BLOCK && $self->{_commonmark_options}{skip_code_block} )
              and next;
...
sub _translate_inline_content_block {
    ...
    my $paragraph = CommonMark->parse_document(
        $translator->(
            CommonMark->create_paragraph( children => \@children )->render_commonmark( $self->_render_options )
        )
    )->first_child;
    $paragraph->next
      and die wrap_mod(
    ...
    $paragraph->get_type == CommonMark::NODE_PARAGRAPH
      or die wrap_mod( 'po4a::commonmark', dgettext( 'po4a', "Unexpected node type %s, expecting paragraph." ), $type );
    ...
    $node->replace( CommonMark->$method( %kept_attributes, children => \@children ) );
    return;
}
...
sub _generate_type {
    ...
    return join( " / ",
        map { _generate_single_type($_) } ( @{ $self->{_container_stack} }, _node_info_for_generating_type($node) ) );
}
...
sub _slurp_and_parse_document {
    ...
    if ( $self->{_commonmark_options}{yaml_metadata} ) {
        ...
        ( defined $line && $chomped eq "---" )
          or die wrap_mod( 'po4a::commonmark',
            dgettext( 'po4a', 'The first line must be triple hyphenes if the yaml_metadata option is enabled.', ) );
        unless (
...
Enabling the C<debug> option causes the program to display libcmark
version information, which makes diagnosing issues easier by providing
more detailed context.

This module doesn't support the C<width> option for wrapping
paragraphs in the output files, unlike the L<Locale::Po4a::Text>
module.  This limitation is intentional; refer to the comment in the
source code.

Some configuration options can be passed using C<option>:
...
Manipulating the abstract syntax tree might make it possible to retain
the original document structure, but that would require additional
(seemingly hard) work.  Please report if there is a strong need for
such functionality.

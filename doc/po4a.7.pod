...
po4a (PO for anything) eases the maintenance of the documentation's
translation using the classical gettext tools. To that extend, the
main feature of po4a is to decouple the translatable content of a
document from its structure.

This document serves as an introduction to the po4a project, particularly useful
to potential users considering whether to use this tool, and to the curious ones
...
everyone. But the licensing is not the only restriction at stake: non-translated
free software is useless for non-English speakers, and we still have some work
to make it available to really everybody out there.

The situation is well-perceived by most actors and everybody is now convinced of
the necessity to translate everything. Yet, the actual translation represent a
huge effort of many individuals, crippled by small technical difficulties.

The open-source software are actually very well translated, thanks to the
...
(called PO files, or translation catalogs). A whole ecosystem of tools emerged
to help the translators actually translating these PO files. The result is then
displayed by gettext at run time to display translated messages to the end
users.

Regarding the documentation, the situation still somewhat disappointing. At
first, translating a documentation may seem easier than translating a program:
you just have to copy the documentation source file, and start translating the
content. But when the original documentation is modified, keeping track of the
modifications quickly turns into a nightmare for the translators. If done
manually, this task reveals unpleasant and error prone.

Outdated translations are often worse than no translation at all: the end-users
are tricked by a documentation describing an old behavior of the program, but
they cannot interact directly with the maintainers since they don't speak
English. Even if they could, the maintainer cannot fix the problem as they don't
master every language in which their documentation is translated. These
difficulties undermines the motivation of volunteer translators, further
aggravating the problem.

B<The goal of the po4a project is ease the work of documentation translators>.
In particular, it makes the documentation translations I<maintainable>.
...
yet is left in English in the resulting document, ensuring that the end users
never see an outdated documentation.

This automatizes most of the grunt work of the translation maintenance.
...
documentation. It is for example used to document the git system, which manpages
are translated using po4a.
...
unfortunately, not the translator one. Until you use po4a.
...
Documents of this length can reveal very challenging to update. B<diff> often
reveals useless when the original text was re-indented after update.
Fortunately, po4a can help you in that process.
...
of them would be easy to support in po4a, and not only documentation ones. This
includes package descriptions (deb and rpm), package installation scripts
questions, package changelogs, and all specialized file formats used by the
...
Here, F<master.doc> an example name for the documentation to be translated;
...
a manpage, a XML document, an asciidoc file or similar); the translators will be
...
Adding new text to the translation is probably the only thing that is easier on
the long run when you translate files manually :) This happens when you want to
add an extra section to the translated document, not corresponding to any
content in the original document. The classical use case is to give credits to
the translation team, and to indicate how to report translation-specific issues.

With po4a, you have to specify B<addendum> files, that can be seen as a sort of
patches applied to the localized document after processing. Each addendum must
be provided as a separate file, which format is however very different from the
classical patches. The first line is a I<header line>, defining the insertion
point of the addendum (with an unfortunately cryptic syntax -- see below) while
the rest of the file is added verbatim at the determined position.
...
#TRANSLATORS: you can keep the French example here, or invert it: Use an header in your own language at the beginning, and then say that it shouldn't be in English, translating the second header example to English

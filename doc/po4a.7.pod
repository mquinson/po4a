...
better, remove the second one from where it is and put the first one in the
place of the second one.
...
authors. So, the addendum will be put between both sections. Note that if later
some other section will be added between NAME and AUTHOR sections, it will
break this example making the addenda to be added before this newly added
section.

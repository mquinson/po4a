...
interestingly, the maintainance of translation) using gettext tools on areas
...
Please refer to L<po4a-translate(1)|po4a-translate(1)> for more details,
like the list of existing modules (which is also documented elsewhere in
this document)
...
language code):
  ...
  $ po4a-translate -t <module> original.file XX.po -o XX.sgml
...
when the original file has changed, use the I<po4a-updatepo> program like that:
  ...
  $ po4a-updatepo -t <module> -i new_original.file -o existing.XX.po

(Please refer to L<po4a-updatepo(1)|po4a-updatepo(1)> for more details)
...
all modules. Only file names and the C<-t> option change.
...
following command:
 ...
 $ po4a-gettextize -t sgml original.old.sgml XX.sgml > XX.po
...
=item position (mandatory when mode=before)
...
=item beginboundary (usefull only when mode=after)

=iten endboundary

regexp matching the end of the section after which we should put the
addendum. These arguments are used only when mode=after. You can give only
one of them. If you give the beginboundary, that means that you want to
place the addendum before the given line, and if you give the endboundary
that means that you want to place the addendum after the given line.

This can seem obscure, but it is needed because some formating language
(like nroff) don't mark the end of sections, and you have to use the
beginning of the next section as boundary, and place the addendum before it.
...
then be added B<after> the next line after the C<position> matching the
C<endboundary>. 
...
then be added B<before> the next line after the C<position> matching the
C<beginboundary>. 

If you want to add something just after a given line, (say "Copyright
blabla"), give a regexp matching anything (ie C<^>) as C<beginboundary>
argument. 
...
and a file named C<extra-content> contains this:
 ...
 |PO4A-HEADER:mode=after;position=AUTEUR;lang=fr;boundary=^head
 |
 |=head1 TRADUCTEUR
 |
 |moi
...
po4a. L<Locale::Po4a::TransTractor(3perl)|Locale::Po4a::TransTractor(3perl)>
...
insert the addendum after this line.
...
L<Locale::Maketext::TPJ13(3perl)|Locale::Maketext::TPJ13(3perl)> man page

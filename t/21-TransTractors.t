...
for my $format (@formats) {
    push @tests,
      ...
      {
        'run' => "PATH/po4a-gettextize -f $format -m t-21-TransTractors/$format -l t-21-TransTractors/$format.fr -L ISO-8859-1 -p tmp/po 2>/dev/null",
        'tests' => ["PODIFF -I^# t-21-TransTractors/$format.po tmp/po"],
        'doc'  => "gettextize $format page with original and translation",
      },
      {
        'run' => "cp t-21-TransTractors/$format.po tmp/po && PATH/po4a-updatepo -f $format -m t-21-TransTractors/$format -p tmp/po >/dev/null 2>&1 ",
        'tests' => ["PODIFF t-21-TransTractors/$format.po tmp/po"],
        'doc'  => "updatepo for $format document",
      },
      {
        'run' => "PATH/po4a-translate -f $format -m t-21-TransTractors/$format -p t-21-TransTractors/$format.po-ok -l tmp/$format.fr",
